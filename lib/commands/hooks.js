...
// v3: Compares HEAD to HEAD~1 to detect changes in the commit just made
const postCommitHookContent = `#!/bin/sh
# Git Pet post-commit hook script v3 - DEBUGGING

echo "[GitPet Hook DEBUG] Starting hook..." >&2

# Get the parent commit SHA (if it exists)
PARENT=$(git rev-parse --verify HEAD~1 2>/dev/null)
echo "[GitPet Hook DEBUG] Parent SHA: '$PARENT'" >&2

# Default: assume changes unless proven otherwise for initial commit
CHANGES_DETECTED=true

if [ -n "$PARENT" ]; then
    # If parent exists, compare HEAD against parent
    echo "[GitPet Hook DEBUG] Parent found. Running: git diff --quiet '$PARENT' HEAD --" >&2
    git diff --quiet "$PARENT" HEAD --
    DIFF_EXIT_CODE=$? # Capture the exit code immediately
    echo "[GitPet Hook DEBUG] git diff exit code: $DIFF_EXIT_CODE" >&2

    # Use --quiet for speed, exit code 1 means differences found
    if [ $DIFF_EXIT_CODE -eq 0 ]; then
        # Exit code 0 means NO differences found
        echo "[GitPet Hook DEBUG] git diff found NO changes." >&2
        CHANGES_DETECTED=false
    else
        # Exit code non-zero (usually 1) means differences WERE found
         echo "[GitPet Hook DEBUG] git diff FOUND changes." >&2
         CHANGES_DETECTED=true # Explicitly set true, though it's default
    fi
else
    # No parent means initial commit, which always has changes
    echo "[GitPet Hook DEBUG] No parent found (initial commit?). Assuming changes." >&2
    CHANGES_DETECTED=true
fi

# Final check before calling react
echo "[GitPet Hook DEBUG] Final CHANGES_DETECTED value: $CHANGES_DETECTED" >&2

if $CHANGES_DETECTED; then
    echo "[GitPet Hook DEBUG] Calling react with --changes" >&2
    git-pet react post-commit --changes || true
else
    echo "[GitPet Hook DEBUG] Calling react without --changes" >&2
