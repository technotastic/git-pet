...
// --- Helper Functions ---
// (awardExp and unlockAchievement functions remain exactly the same as before)
// ... (paste the full awardExp and unlockAchievement functions here from the previous correct version) ...
/**
...
async function handleReactCommand(event, hookArgs = []) {
  ...
  try {
    ...
    console.log(
      chalk.blueBright(
        "[DEBUG react.js handleReactCommand] Received event:",
        event,
        "Args:",
        hookArgs
      )
    ...
    switch (event) {
      case "post-commit":
        ...
        console.log(
          chalk.blueBright(
            "[DEBUG react.js] Post-commit: changesMade flag =",
            changesMade
          )
        );
        ...
        if (changesMade) {
          console.log(
            chalk.blueBright(
              "[DEBUG react.js] Awarding EXP for commit:",
              levelingConfig.rewards.COMMIT_WITH_CHANGES
            )
          );
          ...
          console.log(
            chalk.grey(
              "React: Skipping EXP for commit - hook indicated no file changes."
            )
          );
        }
        break;
      ...
      case "post-merge":
        ...
        console.log(
          chalk.blueBright(
            "[DEBUG react.js] Post-merge: wasConflict flag =",
            wasConflict
          )
        );
      ...
      case "pre-push":
        console.log(
          chalk.blueBright("[DEBUG react.js] Pre-push: awarding EXP")
        );
      ...
      case "branch-deleted":
        ...
        console.log(
          chalk.blueBright(
            `[DEBUG react.js] Branch-deleted: wasOld=${wasOld}, wasMerged=${wasMerged}`
          )
        );
        ...
        if (wasOld || wasMerged) {
          ...
          console.log(
            chalk.grey(
              "React: Skipping EXP for branch delete - hook provided no reason (old/merged)."
            )
          );
        }
        break;
    ...
    if (state) {
      ...
      console.error(
        chalk.red("React: State was not loaded successfully, cannot save.")
      );
    }
  }
}

// --- Yargs Command Definition (Corrected V2) ---
module.exports = {
  command: "react <event> [args..]", // Define command signature with optional args array
  describe:
    "Internal command for reacting to Git events (usually called by hooks)",
  builder: (yargs) => {
    yargs
      .positional("event", {
        describe: "The Git event name",
        type: "string",
        demandOption: true,
      })
      .positional("args", {
        // Explicitly define the args positional collector
        describe: "Optional arguments/flags passed from the hook script",
        type: "array", // Collect remaining positionals into argv.args
        default: [],
      })
      .strict(false) // Still allow unknown flags like --changes without erroring
      .hide("react");
  },
  handler: async (argv) => {
    const event = argv.event;
    // Use argv.args which yargs should populate based on the builder definition
    const hookArgs = Array.isArray(argv.args) ? argv.args : [];

    // Add more detailed debug logging
    console.log(
      chalk.blueBright(`[DEBUG react.js Handler V2] Event: ${event}`)
    );
    console.log(
      chalk.blueBright(
        `[DEBUG react.js Handler V2] Received argv.args: ${JSON.stringify(
          argv.args
        )}`
      )
    );
    console.log(
      chalk.blueBright(
        `[DEBUG react.js Handler V2] Received argv._: ${JSON.stringify(argv._)}`
      )
    ); // Log argv._ too for comparison
    console.log(
      chalk.blueBright(
        `[DEBUG react.js Handler V2] Using hookArgs: ${JSON.stringify(
          hookArgs
        )}`
      )
    );
